...
// ************************** NEW FUNCTION **************************
std::vector<int> GetSentenceIndicesByPMI(const std::vector<std::string>& sentences_vec, float masked_sentence_ratio) {
    // Merge all sentences into a single string
    std::string merged_sentences;
    for (const auto& sentence : sentences_vec) {
        merged_sentences += sentence + " ";
    }
    ...
    PyObject* pModule = PyImport_ImportModule("calc_pmi_list");
    if (!pModule) {
        PyErr_Print();
    ...
    PyObject* pFunc = PyObject_GetAttrString(pModule, "get_pmi_list_from_doc");
    ...
    PyObject* pArgs = PyTuple_Pack(1, PyUnicode_FromString(merged_sentences.c_str()));
    ...
    // Sort sentences based on PMI scores
    std::vector<std::pair<float, int>> sentences_pmi_score;
    for (int i = 0; i < pmi_list.size(); ++i) {
        sentences_pmi_score.push_back(std::make_pair(pmi_list[i], i));
    }
    std::sort(sentences_pmi_score.begin(), sentences_pmi_score.end(), std::greater<>());

    // Calculate the number of sentences to be masked
    const int n_masked_sentences = static_cast<int>(sentences_vec.size() * masked_sentence_ratio);

    // Select the top n_masked_sentences indices
    std::vector<int> indices;
    for (int i = 0; i < n_masked_sentences; ++i) {
        indices.push_back(sentences_pmi_score[i].second);
    }
